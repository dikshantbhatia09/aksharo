...
| B12 | Academy tracks + rewards, Changelog + What's new, Help centre, support tickets with diagnostics | A13, B02 | done pending verification (merging: 4 academy tracks + exactly-once rewards, 10 help articles, /updates changelog + What's new + RSS, support tickets; academy lot source → M03; academy-help Playwright → M03) |
...
| C01 | Local bridge v2: `bridge-core` + Node SEA app, relay-first WSS, loopback HTTPS + per-install cert, pairing, 12 h pair tokens, api relay module | A04, A08, B08 | done (C01 merged; C01b keystore + flagged tray merging; B08b per-device tokens merged) |
