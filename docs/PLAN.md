...
| A23 | e2e suite, seed sample, verify-wave script, X02 load harness | A13–A21 | done (A23 + A23b merged: Gate A journey e2e, sample seed, verify-wave script, harness drains + 40P01 retry; verify-wave run at the gate) |
...
| B13 | Admin console: roles + step-up, users/credits/refunds, flags, styles/parity, routing weights, jobs/DLQ, mandates, TDS, affiliate review, DSR/breach, share reports, metrics | B01–B12, B16 | done (B13 merged; B13b delivered b8a0490 pending merge: routing overrides to the worker, support queue over B12, share-report notify, server-side admin gate, SVG charts) |
...
| B15 | Share/review links (view/comment/approve, hygiene), comments, batch, replace media (re-align), import transcript & align | A12, A21, A10, B08 | done pending merge (ccd2f46: share viewer + panel + batch UI, replace-media re-alignment via SetWordTiming, import-and-align EDG init; share.spec.ts 4/4; ai.align stays 0 credits by ruling) |
...
| C02 | Desktop shell: Electron loading the hosted web app (decision D71), deep links, updater with channels, tray, embedded bridge, hardened defaults | A13, C00, C01 | done (C02 + C02b merged: hardened shell, deep links, updater, real bridge adapter, pairing approval window, device bootstrap, CI e2e job; C00b fixes electron-builder packaging) |
...
| C10 | Installers (NSIS/pkg/Resolve/.ccx), `/plugins/manifest`, plugins + download pages | C00, C02, C05a, C08 | done pending merge (3c505d4: real manifest with cache/fallback, NSIS + dmg/pkg targets, budget check, Resolve uninstallers, marketplace prep (H-24); pages e2e written, runs at Gate B) |
...
| C04 | Local mode: SQLite + files, local export, upload-to-cloud as a new project, Starter+ gate | C03a | done pending merge (27125fc: sql.js store, main-process network guard, Starter+ gate, POST /projects/{id}/edg/import; C04b: transcript chunks locally for word ops + resegment, engine /probe, editor gating copy) |
...
- **Gate B** (procedure: `05-build/_orchestration/GATE-B-CHECKLIST.md`): Razorpay test payment with mandate; credits reserved/settled/reconciled; Rule 46 invoice PDF; affiliate attributed by code and cookie; referral credits; autocut + zoom reviewed and exported in sync.
