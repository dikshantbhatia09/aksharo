...
### Amendment 2026-09-03 (D06 / D04a / D05)
- `PassType` gains `sfx` and `music` (D04c/D05). `ItemKind` gains `sfx` and `music`.
- `TitlePayload` gains optional `intent: "title"|"stat"|"quote"|"hook"`, `motionPreset: "pop"|"slide-up"|"typewriter"|"underline"|"count-up"|"fade"`, `anchorWordIds: string[]`, `layoutHint: { candidate: "top-third"|"upper-left"|"upper-right"|"centre", safeArea: Rect }`. Additive; there is no `text_fx` kind.
- `SfxPayload { assetId, packId, startMs, durationMs, gainDb, fadeInMs, fadeOutMs, duck: { depthDb, attackMs, releaseMs } | null, licenceSnapshot, cueReason }`.
- `MusicPayload { assetId, packId, startMs, durationMs, gainDb, loopPolicy: "none"|"loop"|"trim", bedDuck: { depthDb, attackMs, releaseMs } | null, licenceSnapshot, mood, bpm }`.
...
## 6. Storage keys
Raw (S3): `ws/{workspaceId}/p/{projectId}/media/{mediaId}/raw.{ext}`. Derived (R2): `ws/{workspaceId}/p/{projectId}/media/{mediaId}/{audio16k.wav|audio48k.wav|proxy540.mp4|waveform.json|thumb-{n}.jpg|subtitle.json}` (`subtitle.json` = imported subtitle sidecar as a cue list, added 2026-09-02 after A06); exports `ws/{workspaceId}/p/{projectId}/exports/{exportId}.{ext}`; fonts `ws/{workspaceId}/fonts/{fontId}.{ttf|otf|woff2}`; brand assets (watermarks, logos) `ws/{workspaceId}/brand/{assetId}.png` (added 2026-09-02 after A20). · `ws/{workspaceId}/support/{ticketId}/diagnostics.zip` (C12 diagnostics bundle attached to a support ticket; 10 MB cap; deleted with the ticket — added 2026-09-03)
