...
## 7. Realtime
Rooms `project:{projectId}`, `workspace:{workspaceId}`; events `edg.ops {revision, ops, source}`, `job.progress {jobId, progress, etaMs}`, `job.completed {jobId, status}`, `comment.added`. Bridge relay rooms `bridge:{workspaceId}` (Wave 4).
