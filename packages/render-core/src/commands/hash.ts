 ...
 * Golden hashes.
 *
 * A16 commits a hash per (style × script × timestamp); A18a re-computes it on
 * every backend and fails the build when the browser and the cloud disagree
 * (D33). For that to mean anything the hash has to be stable across machines,
 * Node versions and key insertion order, so the list is canonicalised — object
 * keys sorted, numbers already quantised by `units.q` — before being hashed
 * with a dependency-free SHA-256 that runs in a browser as happily as in Node.
 */
...
/** The golden hash of a command list: SHA-256 over its canonical JSON. */
export function hashCommands(commands: unknown): string {
