...
- **S04: the export clamps where the editor shows.** S01 moved the editor's
  safe-area guides and drag clamp to a percentage of the canvas's short side,
  but `@montaj/render-core` — which both the CanvasKit preview and the Skia cloud
  export run — still took the margin off the canvas _height_, so on a 1080×1920
  frame a 5% setting clamped the rendered caption 96px from the edge while the
  guide sat at 54: the editor and the export disagreed by 42px on every portrait
  project. A new `ofCanvasShortSide` helper now computes that margin for both
  consumers — `layoutSegment`'s safe-area clamp and `fitBudget`'s available
  width and height — so guide, drag clamp, line budget and rendered frame all
  read the same number. Type size is untouched and still scales with the height.
  The margin had been wide enough on portrait canvases to be self-defeating: the
  caption block often did not fit between the two margins at all, so the clamp
  was skipped entirely and the caption sat _outside_ the safe area it was meant
  to respect — of the 120 golden snapshots, 4 violated their own safe area
  before and none do now. The transcription-time budget follows the same
  correction: the default style's Latin line budget at 9:16 goes from 12 to 18
  characters, because the usable width was 542.4px and is now 777.6px. Goldens
  were regenerated by `golden:build`; 16:9 output is byte-identical, as
  `min(w, h)` is the height there.
