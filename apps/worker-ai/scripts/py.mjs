 ...
 *
 * The venv lives at `apps/worker-ai/.venv` and is created on first use, so a
...
function note(message) {
  process.stderr.write(`[worker-ai] ${message}\n`);
}
...
function ensureVenv() {
  ...
  // Editable install so `worker_ai` is importable without touching PYTHONPATH.
