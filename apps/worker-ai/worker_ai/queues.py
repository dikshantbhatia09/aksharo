...
#: `ai.transliterate` from A22, `ai.clean` from B10); the rest are registered
#: and answer `not_implemented` so a producer gets a clear failure instead of a
#: job that sits in Redis forever.
