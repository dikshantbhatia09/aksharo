...
#: `ai.transliterate` from A22, `ai.pass` from B18); the rest are registered and
#: answer `not_implemented` so a producer gets a clear failure instead of a job
#: that sits in Redis forever. `ai.pass` itself still answers
#: `worker/not_implemented` for any `passType` other than `"autocut"`
