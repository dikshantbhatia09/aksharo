"""Script projection for the Indic aligner: Roman to Devanagari.

`09 §2` requires one projection, and it is not a translation:
...
There used to be a second direction, Devanagari to Latin, because Meta MMS ran a
single multilingual **Latin** head and had to be fed transliterated text.
Decision **D77** removed MMS on licence grounds and replaced it with per-language
XLSR-53 fine-tunes, each with its own vocabulary in its own script — so nothing
romanises any more, and the reverse table went with the rung that used it.

This is a **rule table, not a model**. A model — IndicXlit, which A22 owns for the
user-visible transliteration of `09 §4` — would be better at proper nouns and at
schwa deletion. It would also be a download, a licence and a second inference pass
inside an aligner whose entire job is to be cheaper than paying a vendor for word
timings. The rules below are wrong at the margins in ways that cost a *character*,
and a character is 20 ms of a CTC lattice that then re-aligns around it; they are
not wrong in ways that lose a word.
