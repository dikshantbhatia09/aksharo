#!/usr/bin/env node
/**
 * Cross-platform bridge from pnpm/turbo to this app's Python virtual environment.
 *
 *   node scripts/py.mjs -m pytest -q
 *   node scripts/py.mjs --setup-only
 *
 * The venv lives at `apps/model-server/.venv` and is created on first use, so a
 * fresh clone can run `pnpm test` at the repo root without a separate Python
 * setup step. Reinstalls happen only when `requirements-dev.lock` changes,
 * tracked by a hash stamp inside the venv.
 *
...
function note(message) {
  process.stderr.write(`[model-server] ${message}\n`);
}
...
function ensureVenv() {
  ...
  // Editable install so `model_server` is importable without touching PYTHONPATH.
