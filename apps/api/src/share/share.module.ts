 ...
 * (`/s/:token`), password unlock, report-abuse intake and the approve/
 * request-changes decision (B15 brief §1, §3).
 *
 ...
 * `RolesGuard` wiring, same as `ProjectsModule`. `ShareLinksService` is
 * exported because `CommentsModule` (public comment routes) needs the same
 * scope and liveness checks.
 */
@Module({
