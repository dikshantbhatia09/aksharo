...
export class StreakService {
  ...
  async getView(workspaceId: string): Promise<StreakView> {
    const row = await this.ensureAssigned(workspaceId);
    ...
    const discountPercent = row.holdout ? 0 : (discountPercentForLevel(row.level) ?? 0);
    const creditGrantTenths = row.holdout ? 0 : (creditGrantTenthsForLevel(row.level) ?? 0);
  ...
  async getDiscountPercent(workspaceId: string): Promise<number> {
    ...
    if (row === null || row.holdout || row.paused || row.creditsOnly) return 0;
  ...
  async rolloverOne(workspaceId: string, now: Date = new Date()): Promise<void> {
    ...
    const result = rolloverWeek(state, publishDayCount);
    ...
    if (isNewMonth || result.leveledUpTo !== undefined) {
