...
| `product_events` is not in 06                           | B17 brief §2 asks for launch-metrics instrumentation (`13-launch-plan.md`) that 06 has no table for. Append-only, deliberately with no FK relations to `users`/`workspaces` — recording an event must never fail, or be blocked, on an unrelated write.                                                                                                                                                                                                                                                 |
