...
export async function runPackageResolve(ctx: ReleaseContext, config: ReleaseConfig, version: string): Promise<PackageResolveResult> {
  ...
  if (!(await pathExists(path.join(absPluginDir, `${PLUGIN_IDS.resolveScript}.lua`)))) {
    ...
    await fs.writeFile(path.join(sourceDir, `${PLUGIN_IDS.resolveScript}.lua`), "-- placeholder Resolve script (C08 not landed yet)\n", "utf8");
  }
  ...
  const bundlePath = path.join(ctx.outDir, "artifacts", "plugins", `aksharo-resolve-${version}.zip`);
