 ...
 * so esbuild's browser/IIFE bundle (this plugin's UXP target — no Node APIs) fails to resolve
 * those Node builtins even though this file only ever needs the protocol schemas. Test files
 * (which run under vitest/Node) may still import the real `@montaj/bridge-core` directly.
 *
