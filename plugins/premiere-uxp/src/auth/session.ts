/**
 * Sign-in state machine: device-code-style pairing over the bridge (CONTRACTS §5, B08b
 * device flow; bridge pairing per `@montaj/bridge-core`'s `pair.request`/`pair.confirm`/
 * `session.exchange`). The system browser opens for the tray-gesture approval path
 * ("Premiere Pro wants to connect — Allow/Deny", D25); the 8-char code is the fallback when
 * the tray gesture is unavailable (5 attempts, matching THREAT-MODEL T12's lockout — enforced
 ...
 * never in `uxp.storage.secureStorage`, `localStorage`, or written to disk. A panel reload
 * signs the user out; that is intentional, not a gap (see `src/host/premiere.ts`'s header
 * comment for the brief/threat-model conflict this resolves).
 */
...
export class SignInSession {
  ...
  async beginSignIn(clientName = "Aksharo Panel (Premiere Pro)"): Promise<void> {
