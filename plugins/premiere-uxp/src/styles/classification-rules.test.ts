...
describe("SHARED_CLASSIFICATION_RULES", () => {
  it("matches C08b's classification_rules.json when it exists in this worktree", () => {
    ...
    if (!canonicalPath) {
      ...
      // which is only present once wp/C08b has merged into main. Not a failure
      // here — just nothing to compare against yet.
      expect(canonicalPath).toBeUndefined();
      return;
    }
    const canonical = readCanonicalRules(canonicalPath);
    // Compare rule bodies (ids, fields, predicates, status), not the exact
    // reason prose — this module intentionally rewords reasons for AE/Premiere
    // instead of reusing Resolve/Text+ wording (see the module doc comment).
