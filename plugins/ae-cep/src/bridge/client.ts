 ...
 * through whatever the CEP panel's Chromium context gives it (`fetch`/`WebSocket`), so this
 * client takes an injectable `BridgeTransport` and never imports `ws` or Node net APIs — see
 * `src/host/ae.ts`'s rule that only that file touches host/runtime-specific globals (here:
 * only `index.tsx`'s production wiring touches the real `fetch`/`WebSocket`; everything else
 * takes the transport as a parameter).
 */
